...
### Import from file
The editor always starts from the bundled `src/data/` files. A file-picker button that loads an existing `gramPositive.json` / `gramNegative.json` back into the editor would let teachers continue editing previously exported files without having to re-run `npm install` or copy files back manually.
